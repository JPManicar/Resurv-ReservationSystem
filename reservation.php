  ...
  //error handling:
    ...
    <div class="signup-form">
        <form action="includes/reservation.inc.php" method="post">
            ...
            <div class="form-group">
            <label for="guests">Enter Reservation Time</label>
                <input type="time" class="form-control" name="time" placeholder="Reserved Time" required="required">
                <small class="form-text text-muted">Please take note that arriving 1 hour and 30 minutes late will void your reservation.</small>
            </div>
            <div class="form-group">
            <label>Enter number of Guests</label>
        	<input type="number" class="form-control" min="1" max = "20" name="num_guests" placeholder="Guests" required="required">
                <small class="form-text text-muted">Minimum value is 1</small>
            </div>
            <div class = "form-group">
            <label>Table Number</label>
            <select class = "form-control" name = "table_num" placeholder = "Select table" required = "required">
            <option>Select Table</option>
            ';
            try{
                $query = "SELECT * FROM table_table WHERE status = 'Available'";
                $result = mysqli_query($conn, $query);
               // echo '<option>'.$row['table_id'].'</option>';
