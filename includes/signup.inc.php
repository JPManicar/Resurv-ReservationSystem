...
if(isset($_POST['signup-submit'])) {
    ...
    if(empty($username) || empty($email) || empty($password) || empty($passwordRepeat)) {
       ...
       $sql = "SELECT username, user_email FROM user_table WHERE username=? OR user_email=?";
       $stmt = mysqli_stmt_init($conn);
       if(!mysqli_stmt_prepare($stmt, $sql)){           //if user exists
           header("Location: ../index.php?error=error1");
           exit();
       }
       else {
           mysqli_stmt_bind_param($stmt, "ss", $username, $email);     //if username is taken
           ...
           $resultCheck = mysqli_stmt_num_rows($stmt);
            ...
            $sql = "INSERT INTO user_table(username, user_email, password, user_type) VALUES(?, ?, ?, 'customer')";
            $stmt = mysqli_stmt_init($conn);
